...
IC = hFE * IB;                       % Kollektör Current
IE = IC + IB;                        % Emiter Current
...
% Voltage Gain
Av = parallel_res([RE RL]) / ((1/gm) + parallel_res([RE RL]));

% Input Impedance (rπ+(β+1)RE)//R1//R2
Rin = parallel_res([(rPi+(hFE+1)*RE), R1, R2]);

% Output Impedance (RE//RL//(1/gm))
Rout = parallel_res([RE RL 1/gm]);
...
title("Emitter Follower (Common Collector) Calculation");
...
draw_capacitor(x_right + 10, y_res_bottom + 5, 0, "CL", CL);
...
draw_wire([
    ...
    [x_right y_res_bottom+5 x_right+5 y_res_bottom+5];
    [x_right+15 y_res_bottom+5 x_right+20 y_res_bottom+5];
    [x_right+20 y_res_top-5 x_right+20 y_res_center-5];
    [x_right y_center+5 x_right y_center+20]
]);
