...
VCC = 12;    % V
RS  = 50;    % internal resistance of the power source (ohm)

% Resistors

R1 = 330e3;   % bias resistor 1
R2 = 220e3;   % bias resistor 2
RC = 4.7e3;   % collector resistor
RE = 4.7e3;   % emitter resistor
...
f = 2e3   ; % working frequency (2 kHz)
w = 2*pi*f; % rad/s

wp = gm / ((1+gm*RS)*CE); % low cutoff frequency (rad/s)
fp = wp / 2*pi;           % high cutoff frequency (Hz)

% valus in low cutoff frequency
Avp = parallel_res([RC RL]) / (RS + 1/gm);  % Av voltage gain
AvpDb = 20*log10(Avp);                      % as dB

% working values
AvDb = AvpDb - 20*(log10(wp)-log10(w)); % 20 db/dec decreasing
Av = 10^(AvDb/20);                      % seen gain
...
title("Common Base NPN Calculation");
...
draw_power(x_left, y_center+5, 90, "GND");
draw_power(x_left, y_center-5, 90, "Vin");
draw_power(x_right+20, y_res_top-5, 90, "Vout");

draw_npn(x_right, y_center);

draw_resistor(x_center - 5, y_res_bottom + 5, 0, "RS", RS);
...
draw_capacitor(x_left + 20, y_center - 5, 0, "CE", CE);
draw_capacitor(x_center - 5, y_center+5, 0, "CB", CB);
...
draw_wire([
    ...
    [x_center y_res_bottom+5 x_center + 5 y_res_bottom+5]
]);
